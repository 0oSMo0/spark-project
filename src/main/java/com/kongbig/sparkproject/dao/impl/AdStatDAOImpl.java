...
import com.kongbig.sparkproject.model.AdStatQueryResult;
...
/**
 * Describe: 广告实时统计DAO实现类
 * Author:   kongbig
 * Data:     2018/4/20 14:23.
 */
public class AdStatDAOImpl implements IAdStatDAO {
    ...
    public void updateBatch(List<AdStat> adStats) {
        ...
        // 对于需要更新的数据，执行批量更新操作（这里是更新而不是累加）
        String updateSQL = "UPDATE ad_stat SET click_count = ? FROM ad_stat WHERE " +
                "date = ? AND province = ? AND city = ? AND ad_id=?";
        List<Object[]> updateParamsList = new ArrayList<Object[]>();
        for (AdStat adStat : updateAdStats) {
            // 注意放入数组中的参数的顺序要和SQL语句的占位符对应
